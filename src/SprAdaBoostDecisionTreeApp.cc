//$Id: SprAdaBoostDecisionTreeApp.cc,v 1.17 2008-04-02 23:36:45 narsky Exp $
...
void help(const char* prog) 
{
  ...
  cout << "\t-N use only N trees from trained AdaBoost          " << endl;
...
int main(int argc, char ** argv)
{
  ...
  // init
  bool discrete = true;// discrete or continuous decision tree
  string tupleFile;
  int readMode = 0;
  ...
  extern char* optarg;
  //  extern int optind;
  while((c = getopt(argc,argv,"hjM:E:o:a:An:l:y:Q:c:C:P:g:b:iseu:v:f:F:r:N:K:Dt:d:w:V:z:Z:x:q:")) != EOF ) {
    switch( c )
      {
      case 'h' :
	help(argv[0]);
  ...
  if( useTopdown )
    tree.reset(new SprTopdownTree(filter.get(),critOpt.get(),
  ...
  if( nCross > 0 ) {
    ...
    for( int i=0;i<nodeMinSize[0].size();i++ ) {
      ...
      if( useTopdown )
	tree1 = 
	  new SprTopdownTree(filter.get(),critOpt.get(),
			     nodeMinSize[0][i],
			     discrete,bootstrap.get());
      else
	tree1 =
	  new SprDecisionTree(filter.get(),critOpt.get(),
      ...
      SprCut cut;
      if( discrete ) cut = SprUtils::lowerBound(0.5);
      if( !ab1->addTrainable(tree1,cut) ) {
  ...
  SprCut cut;
  if( discrete ) cut = SprUtils::lowerBound(0.5);
  if( !ab.addTrainable(tree.get(),cut) ) {
  ...
  if( nUsedTrees > 0 ) {
    cout << "Will use " << nUsedTrees << " trees instead of " 
	 << trainedAda->nClassifiers() << " trained by AdaBoost." << endl;
    trainedAda->useNClassifiers(nUsedTrees);
  }
