...
SprAbsFilter* SprRootReader::readRootObjects(bool needToCalcWeights)
{
  ...
  for( vector<FileInfo>::const_iterator fileIter = fileObjects_.begin(); 
	...
	TObjArray* branchList =  dynamic_cast <TObjArray*> (tree->GetListOfBranches()->Clone());
    // int branchSize = branchList->GetEntries();
    //     std::cout << branchSize << std::endl;
    //  for(int iBranch=0;iBranch<branchSize;iBranch++){
    //               TBranch* thisBranch = dynamic_cast <TBranch*> (branchList->At(iBranch));
    //               std::cout << thisBranch->GetName() << " ";
    //     }
    //     std::cout << std::endl;
	if(tempLeaf == 0){
