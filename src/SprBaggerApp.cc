//$Id: SprBaggerApp.cc,v 1.9 2008-04-02 23:36:45 narsky Exp $
...
void help(const char* prog) 
{
  ...
  cout << "\t-R use RangeBooster (requires -E, -T, and -S)      " << endl;
  cout << "\t-E reweighting factor for events outside of the boosted range (default=0.5)" << endl;
  cout << "\t-T weight threshold for RangeBooster (default=0.1) " << endl;
  cout << "\t-S signal fraction kept by RangeBooster (default=0.5)" << endl;
...
int main(int argc, char ** argv)
{
  ...
  int baggerMode = 1; // 1 = standard  2 = ArcX4   3 = RangeBooster
  bool split = false;
  ...
  double signalFraction = 0.5;

  // decode command line
  int c;
  extern char* optarg;
  //  extern int optind;
  while((c = getopt(argc,argv,"ha:An:y:Q:BRE:T:S:g:v:f:r:K:DGt:d:w:V:z:")) != EOF ) {
    switch( c )
      {
      case 'h' :
	help(argv[0]);
	...
	break;
      case 'E' :
	epsilon = (optarg==0 ? 0.1 : atof(optarg));
	break;
      case 'T' :
	threshold = (optarg==0 ? 0 : atof(optarg));
	break;
      case 'S' :
	signalFraction = (optarg==0 ? 0.5 : atof(optarg));
	break;
      case 'g' :
  ...
  else if( baggerMode == 3 )
    bagger.reset(new SprRangeBooster(filter.get(),cycles,signalFraction,
